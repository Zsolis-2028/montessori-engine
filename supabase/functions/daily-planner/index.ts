...
serve(async (req) => {
  ...
  try {
    ...
    const { count: attemptCount, error: rateLimitError } = await supabase
      .from("daily_planner_rate_limits")
      .select("id", { count: "exact", head: true })
      .eq("user_id", userId)
      .gte("created_at", windowStart);

    if (!rateLimitError && (attemptCount ?? 0) >= RATE_LIMIT_MAX_ATTEMPTS) {
      logError(FUNCTION_NAME, "RateLimitError", `User ${userId} exceeded daily planner rate limit`);
    ...
    await supabase.from("daily_planner_rate_limits").insert({ user_id: userId });
